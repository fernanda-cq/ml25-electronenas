...
class Network(nn.Module):
    def __init__(self, input_dim: int, n_classes: int) -> None:
        ...
        # Capa 5: 256 -> 512 (NUEVA)
        self.conv5 = nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1)
        self.bn5 = nn.BatchNorm2d(512)
        # Sin pool aquí para mantener tamaño 3x3
        
        # Capa 6: 512 -> 512 (NUEVA)
        self.conv6 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1)
        self.bn6 = nn.BatchNorm2d(512)
        # Sin pool aquí, quedamos en 3x3
        
        # Dimensión de salida: 3x3x512
        out_dim = 3 * 3 * 512
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.pool1(F.relu(self.bn1(self.conv1(x))))  # 48 -> 24
        x = self.pool2(F.relu(self.bn2(self.conv2(x))))  # 24 -> 12
        x = self.pool3(F.relu(self.bn3(self.conv3(x))))  # 12 -> 6
        x = self.pool4(F.relu(self.bn4(self.conv4(x))))  # 6 -> 3
        x = F.relu(self.bn5(self.conv5(x)))              # 3 -> 3 (sin pool)
